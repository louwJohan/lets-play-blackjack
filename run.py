...
def deck():
    """
    Function takes now arguments. Has a list of number and suits. Creates
    a list of 52 lists containing one number and one suite representing
    a card. Shuffles the list returns a list.
    """
...
def deal_cards(cards, number):
    """
    Args: cards(list of 52 lists) and number(int) 
    Number represents the number of time the loop must run.
...
def print_cards(cards, num):
    """
    Args cards(list of lists) and num (int)
    takes elements in list and prints out playing card
    num determines how many times the loop must run
    """
    for i in range(num):
        print(f'''
               ¬¬¬¬¬¬
...
def check_totals(cards, num):
    """
    Args cards(list of lists) num (int)
...
def play_game_validation(player_input):
    """
    Args player_input(string)
    Compares player input to string to ask for another card to stay
    or to exit the game
    """
...
def continue_game_validation(player_input):
    """
    Args player_input(string)
    Compares player input to string to asks if the player wants
    to continue the game. Returns True.
    """
...
The following outline explains the basic rules of standard blackjack,
...
it's fundamentally a two-player game. In blackjack, players don't
play against each other; and they don't co-operate. The only
competition is the dealer.The aim of the game is to accumulate a
...
any ten-point card. This is called a "blackjack", or a natural 21,
and the player holding this automatically wins unless the dealer also
has a blackjack. If a player and the dealer each have a blackjack,
the result is a push for that player. If the dealer has a blackjack,
all players not holding a blackjack lose.

The players' turns
------------------
After the cards have been dealt, the game goes on with each player
taking action - in clockwise order starting to dealer's left.Then the
player can keep his hand as it is(stand) or take more cards from the
deck (hit), one at a time, until either the player judges that the
hand is strong enough to go up against the dealer's hand and stands,
or until it goes over 21, in which case the player immediately loses(busts).
In most places, players can take as many cards as they like, as long
as they don't bust, but some casinos have restrictions regarding this.

The dealer's turn
-----------------
When all players have finished their actions, either decided to stand or
busted,the dealer turns over his hidden hole card. If the dealer has a
natural 21 (blackjack) with his two cards, he won't take any more cards.
All players lose, except playerswho also have a blackjack, in which case
it is a push - the bet is returned to the player.If the dealer doesn't have
a natural, he hits (takes more cards) or stands depending on the value
of the hand. Contrary to the player, though, the dealer's action is
completely dictated by the rules. The dealer must hit if the value
of the hand is lower than 17, otherwise the dealer will stand.Whether or not
the dealer must hit on a soft 17 (a hand of 17 containing an ace being
counted as 11)differs from casino to casino. There might even be blackjack
tables with different rules within the same casino.
...
def player_game(players_cards, cards_total, computer_cards, cards):
    ...
    Loop asks player for 'hit' or 'stay'. Hit deals new card from cards,
    prints card and works out new total. Stay stops loop and works out
    final total. Checks if final total is bigger than 21. Exit restarts
    the programme.
    """
